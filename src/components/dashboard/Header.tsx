...
      try {
        const res = await db.query('SELECT name_ar, name_en FROM jobs WHERE id = $1', [user.job_id]);
...
  const { user, signOut } = useAuth(); // تعديل: استيراد دالة signOut من useAuth
  const { language, toggleLanguage } = useLanguage();
  const [isMenuOpen, setIsMenuOpen] = useState(false);

  const handleLogout = async () => {
    await signOut(); // تعديل: استخدام دالة signOut من useAuth
  };
  
  const getInitials = (nameAr?: string | null, nameEn?: string | null, email?: string | null) => {
    if (language === 'ar' && nameAr) return nameAr.charAt(0);
