...
export default function RiskAssessment() {
  ...
  useEffect(() => {
    async function fetchControlMeasures() {
      ...
      if (cmError) {
        console.error("Error fetching control measures:", cmError.message);
        setControlMeasuresOptions([]);
        setSelectedControlMeasure(null);
      } else {
        const options = cmData?.map((item: any) => ({
          value: item.id,
          label: language === "ar" ? item.name_ar : item.name_en,
        })) || [];
      ...
      // جلب كود الخطر reference من جدول activity
      const { data: actData, error: actError } = await supabase
        .from("activity")
        .select("reference")
        .eq("id", selectedActivity.value)
        .single();
