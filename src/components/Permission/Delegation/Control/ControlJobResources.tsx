// src/components/Permission/Delegation/Control/ControlJobResources.tsx
...
const translations = {
    ar: {
        pageTitle: "نظام تفويض التحكم (موارد الوظائف)",
        pageDesc: "تحديد الخدمات التي تملك هذه الوظيفة حق 'التحكم' فيها (أي القدرة على منحها للآخرين).",
        ...
        save: "حفظ صلاحيات التحكم",
        saving: "جاري الحفظ...",
        successTitle: "تم بنجاح",
        successMessage: "تم تحديث موارد التحكم للوظيفة.",
        errorTitle: "خطأ",
    ...
    en: {
        pageTitle: "Control Delegation (Job Resources)",
        pageDesc: "Define services this job can 'Control' (delegate to others).",
        selectJobTitle: "Select Job",
        searchPlaceholder: "Search job...",
        filterTreePlaceholder: "Search services...",
        noJobsFound: "No jobs found.",
        save: "Save Control Resources",
        saving: "Saving...",
        successTitle: "Success",
        successMessage: "Job control resources updated.",
        errorTitle: "Error",
...
export default function ControlJobResources() {
    const { language } = useLanguage();
    const t = translations[language as keyof typeof translations];
    const { canManageScope, canGrantResource } = useAuth(); 
    const { updateJobControlResources } = useAccessManager(); // ✅ دالة التحكم
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                // فلترة: الوظائف التي أملك "تحكم" عليها
    ...
    // 2. ✅ Fetch Distribution Logic (فلترة النطاقات المتاحة)
    useEffect(() => {
        if (!selectedJobId) {
            setValidCompanies([]);
            setValidSections([]);
    ...
    // 4. Listen to Resources (Control Job Resources Collection)
    useEffect(() => {
        if (!selectedJobId) return;
        const q = query(collection(firestore, 'control_job_resources'), where('job_id', '==', selectedJobId));
    ...
    const getInitialStateCallback = useCallback((id: string) => initialJobResources.has(id) ? true : undefined, [initialJobResources]);
        ...
        try {
            ...
            initialJobResources.forEach((val, key) => {
                if (!jobResources.has(key) && val.docId) {
                    toRemovePromises.push(updateJobControlResources(selectedJobId, {}, 'remove', val.docId));
                }
            });
            ...
            jobResources.forEach((val, key) => {
                ...
                if (!initial || JSON.stringify(initial.scope) !== JSON.stringify(val.scope)) {
                    ...
                    toAddPromises.push(updateJobControlResources(selectedJobId, payload, 'add'));
                }
            });
    ...
    const renderScopeButton = useCallback((node: ServiceNode) => {
        ...
        return (
            <button 
                ...
                className={`p-1.5 rounded-md hover:bg-gray-700 transition-colors ${isScoped ? 'text-red-400 bg-red-500/10' : 'text-gray-600'}`}
                ...
                {!selectedJobId ? (
                    <motion.div key="select" variants={directionalSlideVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
                            ...
                            <div className="relative mb-6">
                                <input type="text" placeholder={t.searchPlaceholder} value={jobSearchTerm} onChange={(e) => setJobSearchTerm(e.target.value)} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg py-3 px-4 pl-10 focus:border-red-500 transition-colors text-white" />
                    ...
                    <motion.div key="editor" variants={directionalSlideVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col flex-1">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mb-6 flex justify-between items-center shadow-lg">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center border-2 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.3)]">
                        ...
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mb-4">
                             <div className="relative">
                                <input type="text" placeholder={t.filterTreePlaceholder} value={treeSearchTerm} onChange={(e) => setTreeSearchTerm(e.target.value)} className="w-full bg-gray-900 border border-gray-600 rounded-lg py-2 px-4 pl-10 focus:border-red-500 text-sm text-white" />
