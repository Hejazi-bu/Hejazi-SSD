// src/components/Permission/Delegation/Access/AccessUserScopes.tsx
...
const translations = {
    ar: {
        pageTitle: "نظام تفويض الوصول (استثناءات المستخدمين)",
        pageDesc: "منح مستخدم محدد صلاحية الوصول إلى نطاقات (شركات/إدارات) إضافية غير مدرجة في وظيفته.",
        ...
        successMessage: "تم تحديث نطاق وصول المستخدم.",
        errorTitle: "خطأ",
    ...
    en: {
        pageTitle: "Access Delegation (User Exceptions)",
        pageDesc: "Grant a specific user access to scopes (Companies/Departments) beyond their job definition.",
        ...
        successMessage: "User access scope updated.",
        errorTitle: "Error",
...
export default function AccessUserScopes() {
    const { language } = useLanguage();
    const t = translations[language as keyof typeof translations];
    
    const { canManageScope } = useAuth();
    const { updateUserAccessScope } = useAccessManager();
    ...
    useEffect(() => {
        ...
        const q = query(collection(firestore, 'access_user_scopes'), where('user_id', '==', selectedUserId));
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const deletePromises = rulesToDelete.map(rule => 
                updateUserAccessScope(selectedUserId, {}, 'remove', rule.docId)
            );
            ...
            const addPromises = rulesToAdd.map(rule => {
                ...
                return updateUserAccessScope(selectedUserId, payload, 'add');
            });
