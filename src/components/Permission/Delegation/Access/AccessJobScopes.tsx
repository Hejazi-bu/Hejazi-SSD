// src/components/Permission/Delegation/Access/AccessJobScopes.tsx
...
const translations = {
    ar: {
        pageTitle: "نظام تفويض الوصول (نطاق الوظائف)",
        pageDesc: "تحديد النطاقات (الشركات/الإدارات) التي يمكن لهذه الوظيفة رؤيتها والوصول إليها.",
        ...
        successMessage: "تم تحديث نطاق الوصول للوظيفة.",
        errorTitle: "خطأ",
        ...
        currentRules: "قواعد النطاق الحالية",
    ...
    en: {
        pageTitle: "Access Delegation (Job Scopes)",
        pageDesc: "Define which scopes (Companies/Departments) this job can see and access.",
        ...
        successMessage: "Job access scope updated.",
        errorTitle: "Error",
        ...
        currentRules: "Current Scope Rules",
        conflictTitle: "External Update",
        conflictMessage: "Scope rules updated externally. Load new data (lose changes)?",
        loadNew: "Load New",
        ignore: "Ignore"
    }
};
...
export default function AccessJobScopes() {
    const { language } = useLanguage();
    const t = translations[language as keyof typeof translations];
    const { canManageScope } = useAuth();
    const { updateJobAccessScope } = useAccessManager();
    ...
    // 2. ✅ Fetch Distribution Logic (فلترة القوائم عند اختيار الوظيفة)
    useEffect(() => {
        if (!selectedJobId) {
            setValidCompanies([]);
            setValidSections([]);
        ...
        fetchDistribution();
    }, [selectedJobId, companies, sections]); // الاعتماد على companies و sections المحملة مسبقاً
    ...
    useEffect(() => {
        ...
        const q = query(collection(firestore, 'access_job_scopes'), where('job_id', '==', selectedJobId));
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const deletePromises = rulesToDelete.map(rule => updateJobAccessScope(selectedJobId, {}, 'remove', rule.docId));
            const addPromises = rulesToAdd.map(rule => {
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                const { isNew, docId, ...cleanRule } = rule;
                const payload: ScopePayload = cleanRule; 
                return updateJobAccessScope(selectedJobId, payload, 'add');
            });
                ...
                {!selectedJobId ? (
                    ...
                    <motion.div key="editor" variants={directionalSlideVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col flex-1">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mb-6 flex justify-between items-center shadow-lg">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center border-2 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)]"><BriefcaseIcon className="w-6 h-6 text-gray-100" /></div>
