// src/components/Permission/Delegation/Access/AccessUserResources.tsx
...
const translations = {
    ar: {
        pageTitle: "نظام تفويض الوصول (موارد المستخدمين)",
        pageDesc: "منح مستخدم محدد حق الوصول إلى موارد (خدمات/صفحات) إضافية كاستثناء.",
        ...
        successMessage: "تم تحديث موارد المستخدم.",
        errorTitle: "خطأ",
        ...
        conflictMessage: "تم تعديل الموارد لهذا المستخدم من مصدر آخر. هل تريد تحميل البيانات الجديدة؟ (ستفقد تعديلاتك)",
        loadNew: "تحميل الجديد",
        ignore: "تجاهل"
    },
    en: {
        pageTitle: "Access Delegation (User Resources)",
        pageDesc: "Grant a specific user access to extra resources (services/pages) as an exception.",
        ...
        successMessage: "User resources updated.",
        errorTitle: "Error",
        ...
        conflictMessage: "Resources updated externally. Load new data? (You will lose current changes)",
        loadNew: "Load New",
        ignore: "Ignore"
    }
};
...
const UserCard = ({ user, onClick, language }: { user: User; onClick: () => void; language: string }) => (
    ...
    >
        <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-gray-700 flex items-center justify-center text-gray-400 group-hover:text-blue-400 transition-colors overflow-hidden border border-gray-600">
...
export default function AccessUserResources() {
    const { language } = useLanguage();
    const t = translations[language as keyof typeof translations];
    const { canManageScope, canGrantResource } = useAuth();
    const { updateUserAccessResources } = useAccessManager();
    ...
    // 2. Listen to Resources
    useEffect(() => {
        if (!selectedUserId) return;
        const q = query(collection(firestore, 'access_user_resources'), where('user_id', '==', selectedUserId));
        ...
        try {
            ...
            initialUserResources.forEach((docId, key) => {
                if (!userResources.has(key)) {
                    toRemovePromises.push(updateUserAccessResources(selectedUserId, {}, 'remove', docId));
                }
            });
            ...
            userResources.forEach((val, key) => {
                if (!initialUserResources.has(key)) {
                    ...
                    toAddPromises.push(updateUserAccessResources(selectedUserId, payload, 'add'));
                }
            });
                ...
                {!selectedUserId ? (
                    <motion.div key="select" variants={directionalSlideVariants} initial="initial" animate="animate" exit="exit" className="space-y-6">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
                            <div className="flex items-center gap-3 mb-2 text-blue-400"><ShieldCheckIcon className="w-8 h-8" /><div><h2 className="text-xl font-bold">{t.pageTitle}</h2><p className="text-xs text-gray-400 mt-1">{t.pageDesc}</p></div></div>
                            <div className="my-6 border-t border-gray-700/50"></div>
                            <div className="relative mb-6"><input type="text" placeholder={t.searchPlaceholder} value={userSearchTerm} onChange={(e) => setUserSearchTerm(e.target.value)} className="w-full bg-gray-900/50 border border-gray-600 rounded-lg py-3 px-4 pl-10 focus:border-blue-500 transition-colors text-white" /><MagnifyingGlassIcon className="w-5 h-5 text-gray-500 absolute left-3 top-1/2 -translate-y-1/2" /></div>
                    ...
                    <motion.div key="editor" variants={directionalSlideVariants} initial="initial" animate="animate" exit="exit" className="flex flex-col flex-1">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 mb-6 flex justify-between items-center shadow-lg">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-gray-700 rounded-full flex items-center justify-center border-2 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.3)] overflow-hidden">
                                ...
                                <div><h2 className="text-lg font-bold text-white">{language === 'ar' ? selectedUserObj?.name_ar : selectedUserObj?.name_en}</h2><p className="text-xs text-blue-400 font-medium">{t.pageTitle}</p></div>
                            </div>
