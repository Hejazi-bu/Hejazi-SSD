...
                const mainQuery = 'SELECT se.*, c.name_ar AS companies_name_ar, c.name_en AS companies_name_en, u.name_ar AS users_name_ar, u.name_en AS users_name_en, j.name_ar AS jobs_name_ar, j.name_en AS jobs_name_en FROM security_evaluations se LEFT JOIN companies c ON se.company_id = c.id LEFT JOIN users u ON se.evaluator_id = u.id LEFT JOIN jobs j ON se.historical_job_id = j.id WHERE se.id = $1';
                const detailsQuery = 'SELECT sed.*, sq.question_text_ar, sq.question_text_en FROM security_evaluation_details sed LEFT JOIN security_questions sq ON sed.question_id = sq.id WHERE sed.evaluation_id = $1 ORDER BY sed.id ASC';
                const historyQuery = 'SELECT ea.*, u.name_ar AS users_name_ar, u.name_en AS users_name_en FROM evaluation_approvals ea LEFT JOIN users u ON ea.approver_id = u.id WHERE ea.evaluation_id = $1 ORDER BY ea.created_at ASC';

                const [mainRes, detailsRes, historyRes] = await Promise.all([
                    db.query(mainQuery, [evaluation.id]),
                    db.query(detailsQuery, [evaluation.id]),
                    db.query(historyQuery, [evaluation.id]),
                ]);
                
...
                        jobs: { name_ar: row.jobs_name_ar, name_en: row.jobs_name_en },
                    };
                    setFullData(structuredData as any);
                }

                const formattedDetails = (detailsRes.rows || []).map(row => ({
                    ...row,
                    security_questions: { question_text_ar: row.question_text_ar, question_text_en: row.question_text_en }
                }));
                setDetails(formattedDetails as any || []);
...
                }));
                
                setEvaluations(formattedData as Evaluation[]);
                
            } catch (error) {
