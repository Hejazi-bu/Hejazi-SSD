...
import { db } from '../../lib/supabaseClient'; // تعديل: استيراد كائن db
...
    try {
      const res = await db.query('SELECT id, page, is_allowed FROM sub_services WHERE service_id = $1 ORDER BY "order" ASC', [service.id]);
...
      const activeSubServices = subServices.filter(ss => ss.is_allowed);
...
      const permittedSubServices = activeSubServices.filter(ss => hasPermission(`ss:${ss.id}`));
