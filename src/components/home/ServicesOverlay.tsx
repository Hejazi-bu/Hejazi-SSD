...
const translations = {
  ar: {
    ...
    noPermissions: "لا توجد لديك صلاحيات على أي خدمة حاليًا.",
  },
  en: {
    services: "Services",
    ...
    noPermissions: "You currently have no permissions for any service.",
  },
};
...
export const ServicesOverlay: React.FC<ServicesOverlayProps> = ({ isOpen, onClose }) => {
  ...
  const { user, updateFavorites, permissions } = useAuth();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const fetchPermittedServices = async () => {
        let permittedServiceIds: number[] = [];

        if (user?.is_super_admin) {
            // --- ✅ تم التصحيح هنا: استخدام is_allowed بدلاً من status ---
            const { data, error } = await supabase
                .from('services')
                .select('id')
                .eq('is_allowed', true); // تم تغيير 'status', 'active'

            if (error) console.error("Error fetching all services for super admin:", error);
            permittedServiceIds = data?.map(s => s.id) || [];
        } else {
            permittedServiceIds = Object.keys(permissions)
                .filter(key => key.startsWith('s:') && permissions[key] === true)
                                ...
                                >
                                    {filteredData.length > 0 ? (
                                        ...
                                        <div className="flex justify-center items-center h-full text-gray-500">
                                            {groupedServices.length === 0 ? t.noPermissions : (activeTab === 'favorites' ? t.noFavorites : t.noResults)}
                                        </div>
                                    )}
                                </motion.div>
                            </AnimatePresence>
